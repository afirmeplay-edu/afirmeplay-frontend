...
import { Check, X, Minus, HelpCircle, Loader2, UserCheck, UserX, ClipboardCheck } from "lucide-react";
import {
  subjectiveTestApi,
...
/** Ordem de ciclo ao clicar numa célula: vazio → SIM → PARCIAL → NAO → BRANCO → vazio. */
const RUBRIC_CYCLE: (SubjectiveRubricValue | null)[] = ["SIM", "PARCIAL", "NAO", "BRANCO", null];

const RUBRIC_STYLES: Record<SubjectiveRubricValue, { label: string; icon: typeof Check; className: string }> = {
  SIM: { label: "Sim", icon: Check, className: "bg-green-500 text-white border-green-500 hover:bg-green-600" },
  PARCIAL: { label: "Parcial", icon: HelpCircle, className: "bg-yellow-400 text-white border-yellow-400 hover:bg-yellow-500" },
  NAO: { label: "Não", icon: X, className: "bg-red-500 text-white border-red-500 hover:bg-red-600" },
  BRANCO: { label: "Branco", icon: Minus, className: "bg-muted text-muted-foreground border-border" },
};

function nextRubricValue(current: SubjectiveRubricValue | null): SubjectiveRubricValue | null {
  const idx = RUBRIC_CYCLE.indexOf(current);
  return RUBRIC_CYCLE[(idx + 1) % RUBRIC_CYCLE.length];
}
...
export function SubjectiveCorrectionMatrix({ testId, classId }: SubjectiveCorrectionMatrixProps) {
  ...
  const handleCellClick = async (questionId: string, studentId: string, current: SubjectiveRubricValue | null) => {
    if (!data) return;
    const key = cellKey(questionId, studentId);
    if (savingCells.has(key)) return;

    const nextValue = nextRubricValue(current);
    ...
    <TooltipProvider>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-muted/30 p-4">
          <div>
            <h3 className="text-base font-semibold text-foreground">{data.test.title}</h3>
          ...
          <table className="w-full min-w-max border-collapse text-sm">
            <thead>
              <tr className="bg-muted/60">
                ...
                {data.questions.map((q) => (
                  <th key={q.id} className="min-w-[64px] border-b border-border p-2 text-center font-semibold text-foreground">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="cursor-help underline decoration-dotted">Q{q.number}</span>
                      </TooltipTrigger>
            ...
            <tbody>
              {data.students.map((student, rowIdx) => (
                <tr key={student.id} className={cn(rowIdx % 2 === 1 && "bg-muted/20")}>
                  ...
                  {data.questions.map((q) => {
                    ...
                    const style = value ? RUBRIC_STYLES[value] : null;
                    const Icon = style?.icon;
                    return (
                      <td key={q.id} className="border-b border-border p-1.5 text-center">
                        <button
                          type="button"
                          disabled={isSaving}
                          onClick={() => handleCellClick(q.id, student.id, value)}
                          title={style?.label ?? "Sem lançamento"}
                          className={cn(
                            "grid h-9 w-9 place-items-center rounded-md border text-xs font-bold transition-all mx-auto",
                            style ? style.className : "border-dashed border-border bg-background hover:border-primary/40 hover:bg-muted/50"
                          )}
                        >
                          {isSaving ? (
                            <Loader2 className="h-3.5 w-3.5 animate-spin" />
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1">
            <span className="grid h-5 w-5 place-items-center rounded bg-green-500 text-white"><Check className="h-3 w-3" /></span> Sim
          </span>
          <span className="flex items-center gap-1">
            <span className="grid h-5 w-5 place-items-center rounded bg-yellow-400 text-white"><HelpCircle className="h-3 w-3" /></span> Parcial
          </span>
          <span className="flex items-center gap-1">
            <span className="grid h-5 w-5 place-items-center rounded bg-red-500 text-white"><X className="h-3 w-3" /></span> Não
          </span>
          <span className="flex items-center gap-1">
            <span className="grid h-5 w-5 place-items-center rounded bg-muted text-muted-foreground border border-border"><Minus className="h-3 w-3" /></span> Branco
          </span>
          <span className="text-muted-foreground/70">Clique na célula para alternar a rubrica.</span>
        </div>
        ...
        <div className="flex flex-col items-stretch gap-3 rounded-xl border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-muted-foreground">
            {finalizeResult ? (
              <span>
                Última finalização: {finalizeResult.processed_count} processado(s), {finalizeResult.skipped_count} sem lançamento,{" "}
                {finalizeResult.error_count} erro(s).
              </span>
            ) : (
              <span>Finalize a correção para calcular nota e proficiência da turma. Pode ser refeito quantas vezes for preciso.</span>
            )}
          </div>
        ...
        {finalizeResult && (finalizeResult.skipped_student_ids.length > 0 || finalizeResult.error_student_ids.length > 0) && (
